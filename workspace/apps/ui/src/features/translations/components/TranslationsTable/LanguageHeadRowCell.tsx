...
const CellInput = memo<{ column: TableColumn<any, any> }>(function RenameLanguageField({ column }) {
...
export const LanguageHeadRowCell = memo<{ column: TableColumn<any, any> }>(function LanguageColumnCell({ column }) {
  const {
    isSourceLanguage,
    isEditing,
    isColumnProcessing,
      ...
      <div className="flex h-7 w-full items-center justify-between">
        {isEditing ? (
          <>
            ...
            <IconButton color="error" name="Trash" variant="solid" onClick={handleRemoveLanguage} />
          </>
        ) : (
          <span className="px-3 first-letter:capitalize">language - {column.label}</span>
        )}
      </div>
      <ButtonGroup className="w-full bg-primary-4">
        <IconButton
