...
export const TranslationsToolbar = () => {
  const { isEditing, storage, handleLoadCsv, handleDownloadCsv } = TranslationsViewContext.use((s) => ({
  ...
  return (
    <div className="flex justify-between gap-2">
      <div className="ml-auto flex items-end gap-1">
        <IconButton variant="solid" name="Upload" className="h-9.5 min-w-20" onClick={handleLoadCsv}>
          Load CSV
        </IconButton>
        <Card label="csv content" compact className="py-1 pr-1 pl-2" color={storage ? "info" : "error"}>
          {storage ? (
            <div className="flex items-center gap-2">
              <span>Version:</span>
              <span>
                {storage?.version && new Date(storage.version).toLocaleString()} - {storage?.filename}
              </span>
              <IconButton
                color="success"
                name="Download"
